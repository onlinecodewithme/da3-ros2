...
: Node("depth_anything_v3", node_options)
{
  ...
  // Synchronized subscribers for image and camera_info
...
void DepthAnythingV3Node::onImageDebug(const sensor_msgs::msg::Image::ConstSharedPtr & msg)
{
  ...
  RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000, 
    "[DEBUG] Received image #%d, size: %u x %u, encoding: %s, timestamp: %d.%09d", 
    image_count, msg->width, msg->height, msg->encoding.c_str(), msg->header.stamp.sec, msg->header.stamp.nanosec);
}
